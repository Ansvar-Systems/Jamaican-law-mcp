#!/usr/bin/env tsx
/**
 * Jamaican Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://moj.gov.jm';
const PORTAL_NAME = 'Ministry of Justice (Jamaica)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/jamaican-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Jamaican Law MCP — Data Freshness Check');
