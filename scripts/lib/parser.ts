/**
 * HTML parser for Jamaican legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Jamaican legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Jamaican statute.
 *
 ...
 *   </div>
 */
export function parseJamaicanHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Jamaican acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Jamaican legal text.
 *
 * Jamaican definitions typically use patterns like:
