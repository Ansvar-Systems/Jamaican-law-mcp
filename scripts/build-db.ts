#!/usr/bin/env tsx
/**
 * Database builder for Jamaican Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Jamaican Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Jamaican statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'JM');
    insertMeta.run('source', 'laws.moj.gov.jm');
