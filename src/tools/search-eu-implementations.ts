/**
 * search_eu_implementations — Search EU directives/regulations referenced by Jamaican legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_jamaican_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as jamaican_statute_count
  ...
  if (input.has_jamaican_implementation) {
    sql += ' HAVING jamaican_statute_count > 0';
  }
