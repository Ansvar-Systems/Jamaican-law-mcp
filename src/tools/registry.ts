/**
 * Tool registry for Jamaican Law MCP Server.
...
import { getJamaicanImplementations, type GetJamaicanImplementationsInput } from './get-jamaican-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'Call this FIRST when you need to understand what Jamaican legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Jamaican statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Jamaican statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Jamaican legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Jamaican statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Jamaican?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Jamaican legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Jamaican statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Jamaican statute references or aligns with. ' +
      'Jamaican laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Jamaican statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_jamaican_implementations',
    description:
      'Find all Jamaican statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Jamaican statutes. ' +
      'Jamaican may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Jamaican legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_jamaican_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Jamaican legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Jamaican statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Jamaican statute or provision. ' +
      ...
      'Jamaican is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Jamaican statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_jamaican_implementations':
          result = await getJamaicanImplementations(db, args as unknown as GetJamaicanImplementationsInput);
          break;
