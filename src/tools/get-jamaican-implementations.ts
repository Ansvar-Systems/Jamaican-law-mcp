/**
 * get_jamaican_implementations — Find Jamaican statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetJamaicanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface JamaicanImplementationResult {
  document_id: string;
...
export async function getJamaicanImplementations(
  db: InstanceType<typeof Database>,
  input: GetJamaicanImplementationsInput,
): Promise<ToolResponse<JamaicanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as JamaicanImplementationResult[];
